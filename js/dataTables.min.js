...
_fnTargetCoords:function(b){var a=d(b).parents("tr")[0];return{x:d("td",a).index(b),y:d("tr",a.parentNode).index(a)}},_fnUpdateBorder:function(b,a){var c=this.s.border.width,e=d(b).offset(),f=d(a).offset();b=e.left-c;var h=f.left+d(a).outerWidth(),g=e.top-c,j=f.top+d(a).outerHeight(),k=f.left+d(a).outerWidth()-e.left+2*c;a=f.top+d(a).outerHeight()-e.top+2*c;if(this.s.dt.oScroll.sY!==""){c=d(this.s.dt.nTable.parentNode).offset();e=d(this.s.dt.nTable.parentNode).scrollTop();f=d(this.s.dt.nTable.parentNode).scrollLeft();
...
b.target.nodeName.toLowerCase()=="td"?b.target:d(b.target).parents("td")[0];if(this.s.columns[this._fnTargetCoords(a).x].enable){var c=this.dom.filler;if(b.type=="mouseover"){this.dom.currentTarget=a;this._fnFillerPosition(a);c.style.display="block"}else if(!b.relatedTarget||!b.relatedTarget.className.match(/AutoFill/))c.style.display="none"}}},_fnFillerPosition:function(b){var a=d(b).offset(),c=this.dom.filler;c.style.top=a.top-this.s.filler.height/2-1+d(b).outerHeight()+"px";c.style.left=a.left-
this.s.filler.width/2-1+d(b).outerWidth()+"px"}};AutoFill.prototype.CLASS="AutoFill";AutoFill.VERSION="1.1.2";AutoFill.prototype.VERSION=AutoFill.VERSION})(jQuery);