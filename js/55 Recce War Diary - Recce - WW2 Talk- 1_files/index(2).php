...
{ipb.global.popups['conact'].kill();}
ipb.global.popups['conact']=new ipb.Popup('confirm',_options);});}},tooltip:function(element,options){options=Object.extend({template:new Template("<div class='ipsTooltip' id='#{id}' style='display: none'><div class='ipsTooltip_inner'>#{content}</div></div>"),position:'auto',content:$(element).readAttribute("data-tooltip").escapeHTML(),animate:true,overrideBrowser:true,delay:0.4},options);var show=function(e){if(options.delay&&!options._still_going){return;}
if(!options.content){return;}
var id=$(element).identify();if(!$(id+'_tooltip')){$(document.body).insert({'bottom':options.template.evaluate({'id':id+'_tooltip','content':options.content})});}
if(options.overrideBrowser&&$(element).hasAttribute('title')){$(element).writeAttribute("data-title",$(element).readAttribute('title').escapeHTML()).writeAttribute("title",false);}
...
{}
return;}};ipb.quickpm.init();;var IPS_BBCODE_POPUP=null;var IPS_URL_STORE={};var IPS_SIZE_ARRAY={1:8,2:10,3:12,4:14,5:18,6:24,7:36,8:48};var ipsBbcodeTags={};if(!Array.prototype.indexOf)
...
{break;}}
if(this.tagStack.length==1){Debug.write('0');this.tree.push(this.tagStack.pop());}else{Debug.write('1');this.tagStack[this.tagStack.length-2].children.push(this.tagStack.pop());}}else{}}else{}
...
while(this.tagStack.length>0){if(this.tagStack.length==1){this.tree.push(this.tagStack.pop());}else{this.tagStack[this.tagStack.length-2].children.push(this.tagStack.pop());}}
Debug.dir(this.tree);},locateTagAfter:function(offset){var foundAt=-2;while(foundAt==-2&&offset<=this.text.length){foundAt=this.text.substr(offset).indexOf(this.options.openSymbol);if(foundAt==-1){return false;}
...
{var tagName=this.text.substr(offset+foundAt+1,endAt);Debug.write("Found "+tagName);var ok=true;var test='';if(match=tagName.match(/^(?:\/)?(\S+?)(\s|=|$)/i))
{test=match[1].toLowerCase();}
if(ipsBbcodeTags.indexOf(test)==-1&&IPS_DEFAULT_TAGS.indexOf(test)==-1)
{ok=false;offset=offset+foundAt+1;foundAt=-2;}
else if(tagName.substr(0,1)!="/"&&(tagName.match(/\s/)&&!tagName.match(/=/)))
{ok=false;offset=offset+foundAt+1;foundAt=-2;}
if(ok==true)
{Debug.write("Confirmed "+tagName);return{data:this.tagToObject(tagName),start:(offset+foundAt),end:(offset+foundAt+endAt+2),children:[]};}}}
...
Debug.dir(IPS_URL_STORE);text=text.replace(/<cite.+?>.+?<\/cite>/g,'');Debug.write("preToBBConversion: "+text);return text;},
/*! postBBConversion */
...
Debug.write("postToBBConversion: "+text);return text;},
/*! preTotHtmlConversion */
preToHtmlConversion:function(text)
{text=text.replace(/(\r\n|\r)/g,"\n");var _matches=phpjs.preg_match_all('\\[url=((?!\\[\\/url\\]).+)\\[\\/url\\]',text);if(_matches.length)
{$(_matches).each(function(i)
{if(i.length)
{if(i[1])
{Debug.write('**'+i[1]);var _PossibleTags=phpjs.preg_match_all('\\[([\\d\\w]+?)(\\s|=)?\\]',i[1]);var _final=i[0];if(_PossibleTags.length)
{Debug.dir(_PossibleTags);$(_PossibleTags).each(function(t)
{if(ipsBbcodeTags.indexOf(t[1])==-1&&IPS_DEFAULT_TAGS.indexOf(t[1])==-1)
...
var noParse=(typeof(CKEDITOR.config.IPS_BBCODE_NOPARSE)!='undefined'&&CKEDITOR.config.IPS_BBCODE_NOPARSE instanceof Array)?CKEDITOR.config.IPS_BBCODE_NOPARSE:[];noParse.push('code');noParse.each(function(tag)
...
{text=text.replace(new RegExp("(?:[\n\xA0]{1,})"+'(\\\[/'+tag+'\\\])','gi'),'$1');}
if(arr.indexOf('a')!=-1)
{text=text.replace(new RegExp('\\\[/'+tag+'\\\]([ ]+?)?'+"(?:[\n\xA0]{1,})",'gi'),'[/'+tag.replace(/\\/g,'')+']');}});text=text.replace(/&(#[0-9]{3,4}|[a-zA-Z]{2,5});/g,'&amp;$1;');text=text.replace(/</g,'&lt;');text=text.replace(/>/g,'&gt;');text=text.replace(/&sect(?!;)/g,'&amp;sect');text=text.replace(new RegExp('\\\[img\\\]([^\\\[]+?)\\\[/img\\\]','gi'),'[img=$1]');Debug.write("preToHtmlConversion: "+text);return text;},
/*! processNoParseTags */
...
{Debug.write(id);o=map['open'][id];c=map['close'][id]-o;if(c>0)
{slice=phpjs.substr(text,o,c);slice=slice.replace(/\$/g,'&#36;');var _origLength=phpjs.strlen(slice);if(_origLength>0)
{slice=slice.replace(/\[/g,'&#91;');slice=slice.replace(/\/(\w+?)\]/g,'/$1&#93;');var _newLength=phpjs.strlen(slice);text=phpjs.substr_replace(text,slice,o,c);if(_newLength!=_origLength)
...
{text=text.replace(new RegExp("(?:\n){1,}"+'([ ]+?)?<'+tag+'(>| )','gi'),'$1<'+tag+'$2');}
if(arr.indexOf('a')!=-1)
{text=text.replace(new RegExp('</'+tag+'>([ ]+?)?'+"(?:\n){1,}",'gi'),'</'+tag+'>');}});text=ipb.textEditor.codeToSmilies(text);text=text.replace(/\n/g,'<br>');Debug.write('postToHtmlConversion: '+text);return text;}};
/*! ============= PARSER START */
var IPS_DEFAULT_TAGS=['right','left','center','b','i','u','url','img','quote','indent','snapback','list','strike','s','sub','sup','email','color','size','font','*'];var myParser=new BBCode({tagsSingle:['img','sharedmedia','attachment','member']});document.observe("dom:loaded",function(){$H(CKEDITOR.config.IPS_BBCODE).each(function(bbcode){if(bbcode.value.single_tag==1)
{myParser.options.tagsSingle.push(bbcode.value.tag);}});ipsBbcodeTags=getKeys(CKEDITOR.config.IPS_BBCODE);try
...
color[i]=String('0'+color[i]).slice(-2);return'#'+color.join('');});};var _sharelinks=window.IPBoard;_sharelinks.prototype.sharelinks={popups:null,bname:null,url:null,title:null,maxTwitterLen:115,init:function()
...
{var globalRegex=new RegExp(regex,'g');var globalMatch=haystack.match(globalRegex);matchArray=new Array();try
{$(globalMatch).each(function(i)
{nonGlobalRegex=new RegExp(regex);nonGlobalMatch=i.match(nonGlobalRegex);matchArray.push(nonGlobalMatch);});}catch(err){}
...
{var find=_matches[0].regExpEscape();var replace='<{{'+_c+'}}>';_matches[0]=_matches[0].replace(/\$/g,'&#36;');codeBlocks[_c]=_matches[0];text=text.replace(new RegExp(find,'g'),replace);_c++;}
if($(IPS_smiles.emoticons))
{$(IPS_smiles.emoticons).each(function(grin)
{if(grin.text!==null)
...
{text=text.replace(new RegExp('<\\{\\{'+_c+'\\}\\}>','g'),codeBlocks[_c]);_c++;});return text;},ckPre:function(text)
...
{text=text.replace(/&lt;/g,'<');text=text.replace(/&gt;/g,'>');text=text.replace(/&amp;/g,'&');Debug.write('**'+text);if(!$(ipb.textEditor.htmlCheckbox)||!$(ipb.textEditor.htmlCheckbox).checked)
{}
return text;},pad:function(n)
{return("0"+n).slice(-2);},convertQuotes:function(text)
{text=text.replace(/(<blockquote class=['"]ipsBlockquote['"]\s*(data-author=['"](.+?)['"])?\s*(?:data-cid=['"]\d+?['"])?\s*(data-time=['"](\d+?)[\'"])?\s*>)/g,function(whole,blockquote,_author,author,_date,date,offset)
{if(text.substr(offset+whole.length,5)=='<cite')
{return whole;}
if(date)
{if(date==parseInt(date)&&date.length==10)
{var _tz=new Date().getTimezoneOffset();var _date=new Date().getDateAndTime(parseInt(date)+parseInt(_tz));date=_date['date']+' '+_date['monthName']+' '+_date['year']+' - '+_date['hour']+':'+_date['min'];}}
if(author&&date)
...
Debug.dir(this.options);if(this.options.isHtml==1)
...
$$('body').first().insert(new Element('input',{type:'text',id:'hte_'+this.editorId,style:'position:absolute;left:-1000px'}));_this=this;ev.editor.on('key',function(e){if(e.data.keyCode==8||e.data.keyCode==37||e.data.keyCode==38)
...
{if(e.data.keyCode==8&&!IPSCKTools.getSelectionHtml(ev.editor))
{next=tag.getNext();if(next==null)
{next=CKEDITOR.dom.element.createFromHtml('<p>&nbsp;</p>');next.insertAfter(tag);}
tag.getFirst().remove();tag.moveChildren(next);tag.remove();}
...
var setTimeouts=true;if(e.data.keyCode==46)
...
{if(ranges[0].getCommonAncestor(true,false).getName()=='p')
{ranges[0].getCommonAncestor(true,false).remove();setTimeouts=false;e.preventDefault();return false;}}catch(e){}}}
if(setTimeouts)
...
{next=tag.getNext();if(next==null)
{next=CKEDITOR.dom.element.createFromHtml('<p>&nbsp;</p>');next.insertAfter(tag);}
tag.moveChildren(next);tag.remove();}
